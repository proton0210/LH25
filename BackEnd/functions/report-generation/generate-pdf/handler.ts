...
export const handler = async (event: GeneratePDFInput): Promise<GeneratePDFInput & { pdfBuffer: string }> => {
  ...
  try {
    ...
    doc.fontSize(24)
       .fillColor('white')
       .text('Lambda Real Estate Pro', 60, 70);
    
    doc.fontSize(16)
       .fillColor('white')
       .text(event.input.reportType.replace(/_/g, " ").toUpperCase() + ' REPORT', 60, 100);
    
    // Property details section
    doc.fillColor('black')
       .fontSize(20)
       .text(event.input.title, 50, 160);
    
    doc.fontSize(12)
       .fillColor('#666')
       .text(`${event.input.address}, ${event.input.city}, ${event.input.state} ${event.input.zipCode}`, 50, 190);
    
    // Key metrics box
    doc.rect(50, 220, pageWidth, 80)
       .fill('#f8f9fa');
    ...
    doc.fillColor('black')
       .fontSize(14)
       .font('Helvetica-Bold')
       .text(event.input.propertyType.replace(/_/g, " "), 70 + metricsSpacing, metricsY + 15);
    ...
    doc.fillColor('black')
       .fontSize(14)
       .font('Helvetica-Bold')
       .text(`${event.input.squareFeet.toLocaleString()} sq ft`, 70 + metricsSpacing * 2, metricsY + 15);
    ...
    doc.fillColor('black')
       .fontSize(14)
       .font('Helvetica-Bold')
       .text(`${event.input.bedrooms} / ${event.input.bathrooms}`, 70 + metricsSpacing * 3, metricsY + 15);
    ...
    if (event.recommendations) {
      ...
      doc.fontSize(11)
         .fillColor('black')
         .text(event.recommendations, 50, 80, {
           width: pageWidth,
           align: 'justify'
         });
    }
    ...
    doc.fontSize(10)
       .fillColor('#666')
       .text(`Generated on ${new Date().toLocaleDateString()} by Lambda Real Estate Pro`, 50, doc.page.height - 70, {
         width: pageWidth,
         align: 'center'
       });
